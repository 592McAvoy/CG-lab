...
bool ShadowVolumeTechnique::Init()
{
    ...
    if (!AddShader(GL_VERTEX_SHADER, "shaderFile/shadow_volume.vs")) {
        return false;
    }

    if (!AddShader(GL_GEOMETRY_SHADER, "shaderFile/shadow_volume.gs")) {
        return false;
    }

    if (!AddShader(GL_FRAGMENT_SHADER, "shaderFile/shadow_volume.fs")) {
        return false;
    }
