...
bool BillboardTechnique::Init()
{
    ...
    if (!AddShader(GL_VERTEX_SHADER, "shaderFile/billboard.vs")) {
        return false;
    }

    if (!AddShader(GL_GEOMETRY_SHADER, "shaderFile/billboard.gs")) {
        return false;
    }
    
    if (!AddShader(GL_FRAGMENT_SHADER, "shaderFile/billboard.fs")) {
        return false;
    }
