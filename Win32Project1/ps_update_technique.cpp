...
bool PSUpdateTechnique::Init()
{
    ...
    if (!AddShader(GL_VERTEX_SHADER, "shaderFile/ps_update.vs")) {
        return false;
    }

    if (!AddShader(GL_GEOMETRY_SHADER, "shaderFile/ps_update.gs")) {
        return false;
    }

	if (!AddShader(GL_FRAGMENT_SHADER, "shaderFile/ps_update.fs")) {
		return false;
	}
